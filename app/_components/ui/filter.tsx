...
export default function FilterSidebar() {
  ...
  return (
    <aside className="col-span-1 border-r pr-4 flex flex-col gap-6">
      {filters.map((filter) => (
        <div key={filter.id} className="flex flex-col gap-3">
          <h3 className="text-lead mb-2">{filter.label}</h3>
            ...
            {filter.options.map((option) => {
              ...
              return (
                <li key={option.value}>
                  <Link
                    href={`?${query}`}
                    className={`flex items-center gap-2 rounded-md px-2 py-1 transition text-body-medium text-center ${
